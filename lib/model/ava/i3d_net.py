...
class I3D(nn.Module):
    ...
    def __init__(self, num_classes=400, dropout_drop_prob = 0.5, input_channel = 3, spatial_squeeze=True):
        ...
        self.features = nn.Sequential(OrderedDict(
            [('Conv3d_1a_7x7',BasicConv3d(input_channel, 64, kernel_size=7, stride=2, padding=3)), # (64, 32, 112, 112)])
             ('MaxPool3d_2a_3x3',nn.MaxPool3d(kernel_size=(1,3,3), stride=(1,2,2), padding=(0,1,1))),
             ('Conv3d_2b_1x1',BasicConv3d(64, 64, kernel_size=1, stride=1)),
             ('Conv3d_2c_3x3',BasicConv3d(64, 192, kernel_size=3, stride=1, padding=1)),
             ('MaxPool3d_3a_3x3',nn.MaxPool3d(kernel_size=(1,3,3), stride=(1,2,2), padding=(0,1,1))),
             ('Mixed_3b',Mixed_3b()), # (256, 32, 28, 28)
             ('Mixed_3c',Mixed_3c()),# (480, 32, 28, 28)
             ('MaxPool3d_4a_3x3',nn.MaxPool3d(kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=(1, 1, 1))),# (480, 16, 14, 14)
             ('Mixed_4b',Mixed_4b()),# (512, 16, 14, 14)
             ('Mixed_4c', Mixed_4c()),# (512, 16, 14, 14)
             ('Mixed_4d',Mixed_4d()),# (512, 16, 14, 14)
             ('Mixed_4e',Mixed_4e()),# (528, 16, 14, 14)
             ('Mixed_4f',Mixed_4f()),# (b,832, 16, 14, 14)

             ('MaxPool3d_5a_2x2',nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2), padding=(0, 0, 0))), # (832, 8, 7, 7)

             ('Mixed_5b',Mixed_5b()),# (b,832, 8, 7, 7)
             ('Mixed_5c', Mixed_5c()), # (b,1024, 8, 7, 7)

             ('avgPool3d',nn.AvgPool3d(kernel_size=(2, 7, 7), stride=1)),# (1024, 8, 1, 1)
             ('dropout',nn.Dropout3d(dropout_drop_prob)),
             ('Logits',nn.Conv3d(1024, num_classes, kernel_size=1, stride=1, bias=True))# (400, 8, 1, 1)
                         ]))
    ...
    def forward(self, x):
        logits = self.features(x) # [b, class, L/16, 1, 1])

        if self.spatial_squeeze:
            logits = logits.squeeze(3)
            logits = logits.squeeze(3) # ([2, 80, 7])

        averaged_logits = torch.mean(logits, 2)  # ([2, 80])
        predictions = self.softmax(averaged_logits) # ([2, 80])

        return predictions, averaged_logits

# a = variable(torch.rand((4,3,64,224,224))).cuda()
# net = I3D()
...
#     start = time.time()
#     b = net(a)
#     print('epoch time = {}'.format(str((time.time() - start) * 1000)))